...
public class WaterBottleLoadingRenderer extends LoadingRenderer {
    ...
    int startDegree;
    private static final int DEFAULT_BOTTLE_COLOR = Color.parseColor("#585252");
  ...
  //  private final List<WaterDropHolder> mWaterDropHolders = new ArrayList<>();
    ...
    private float mStrokeWidthWater;
    ...
    public void updateWater(int degree,String color){
        startDegree = degree;
        mWaterColor = Color.parseColor(color);
    ...
    private void init(Context context) {
        ...
        mStrokeWidthWater = DensityUtil.dip2px(context,0.8f);
    ...
    protected void draw(Canvas canvas, Rect bounds) {
        ...
        mPaint.setStrokeWidth(mStrokeWidthWater);
...
//        mWaterBounds.set(mBottleBounds.left + mStrokeWidth * 1.5f + changeX, mBottleBounds.top + mWaterLowestPointToBottleneckDistance + changeY,
//                mBottleBounds.right - mStrokeWidth * 1.5f - changeX, mBottleBounds.bottom - mStrokeWidth * 1.5f);
        mWaterBounds.set(arcBounds.centerX() - mBottleWidth / 2.0f + mStrokeWidthWater*1.3f  , arcBounds.centerY() - mBottleHeight / 2.0f + mStrokeWidthWater*1.3f ,
                arcBounds.centerX() + mBottleWidth / 2.0f - mStrokeWidthWater *1.3f , arcBounds.centerY() + mBottleHeight / 2.0f - mStrokeWidthWater*1.3f  );
...
//        mPaint.getTextBounds(LOADING_TEXT, 0, LOADING_TEXT.length(), mLoadingBounds);
    ...
    private Path createWaterPath(RectF waterRect, float progress) {
        ...
        float bottleneckWidth = waterRect.width() * 0.3f;
        float bottleneckHeight = waterRect.height() * 0.415f;

        float radius = (waterRect.width() - mStrokeWidthWater) / 2.0f;
        float centerY = waterRect.bottom - 0.81f * radius;

        float changeLength = ( radius * (float) Math.sin(Math.PI*startDegree/180) +mStrokeWidthWater/2 )*2.0f;


        //draw the left side of the bottle's body
        RectF bodyRect = new RectF(waterRect.left, centerY - radius, waterRect.right, centerY + radius);
        path.addArc(bodyRect, 270-startDegree, startDegree - 150);
        ...
        //draw the right side of the bottle's body
        path.addArc(bodyRect, 60, startDegree - 150);


        float cubicXChangeSize = changeLength * mProgress;
        float cubicYChangeSize = startDegree ;
//        float left = waterRect.centerX() - changeLength / 2.0f;
//        float right = waterRect.centerX() + changeLength / 2.0f;
//        float top = centerY - radius;
//        float bottom = centerY + radius;
        path.rQuadTo(-cubicXChangeSize,
                -12.0f
                ,-changeLength,
                0);

//        path.rCubicTo(-changeLength * 0.2f,
//                 -cubicYChangeSize,
//                -changeLength * 0.4f,
//                2*cubicYChangeSize-6.0f,
//                -changeLength,
//                mStrokeWidth*0.5f);
...
//        Log.e("xxxxxxxxxxx water rec",waterRect.top +"  "+waterRect.left+" "+waterRect.bottom+" "+waterRect.right);
